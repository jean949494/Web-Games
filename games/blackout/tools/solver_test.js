...
function solve(room, startPos, target, impactLimit) {
...
for (let s = 0; s < SEEDS; s++) {
  for (let i = 0; i < ROOMS; i++) {
    ...
    const r1 = solve(room, { x: room.spawn.x, y: room.spawn.y - 4 }, room.switchPos, C.IMPACT_LIMIT_MILD);
    if (r1.ok) {
      solvable++;
    } else {
      unsolvable++;
      failures.push({ seed: 9000 + s, room: i, archetype: room.archetype, reason: r1.reason + ' (naechster Abstand ' + r1.bestDist + 'px)', spawn: room.spawn, sw: room.switchPos });
    }
  }
}
